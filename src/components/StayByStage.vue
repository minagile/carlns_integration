    ...
    <div class="con">
      <el-table :data="tableData1" :show-header="false" style="width: 100%" :row-style="{'height': '94px'}">
        ...
        <el-table-column prop="car_nameplate" label="车牌号：浙XXXXXX">
          <template  plate slot-scope="scope">
        ...
        <el-table-column align="center">
          <template slot-scope="scope">
            <el-button type="primary" round plain size="small" v-if="scope.row.type === '1'" @click="$router.push({name: 'DetailP', query: {id: scope.row.id}})">去付款</el-button>
            <el-button type="primary" round plain size="small" v-if="scope.row.type === '2'" @click="$router.push({name: 'DetailC', query: {id: scope.row.id, batch: scope.row.car_batch}})">去付款</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
...
export default {
  ...
  methods: {
    getData () {
      this.$fetch('/fd/index/countWork', {status: 2}).then(res => {
        this.tableData = res
        this.tableData1 = res
      })
    },
