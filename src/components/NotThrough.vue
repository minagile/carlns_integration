    ...
    <div class="con">
      <el-table :data="tableData1" :show-header="false" style="width: 100%" :row-style="{'height': '94px'}">
        ...
        <el-table-column align="center">
          <template slot-scope="scope">
            <el-button type="primary" v-if="scope.row.type === '2'" round plain size="small" @click="$router.push({name: 'NDetailC', query: {id: scope.row.id, batch: scope.row.car_batch, route: 'nothrough'}})">查看详情</el-button>
...
export default {
  ...
  methods: {
    ...
    getData () {
      this.$fetch('/fd/index/countWork', {status: 0}).then(res => {
        this.tableData = res
        this.tableData1 = res
      })
    },
