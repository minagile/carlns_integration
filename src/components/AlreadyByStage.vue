      ...
      <el-table :data="tableData" :show-header="false" style="width: 100%" :row-style="{'height': '94px'}">
        ...
        <el-table-column align="center" width="120">
          <template slot-scope="scope">
            ...
            <el-button type="primary" round plain size="small" v-if="scope.row.type === 2" @click="$router.push({name: 'ADetailC', query: {id: scope.row.id, batch: scope.row.batch}})">查看详情</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
...
export default {
  ...
  methods: {
    getData (status) {
      this.$fetch('/fd/index/selectSurrenderByStagesAd', {status: status}).then(res => {
